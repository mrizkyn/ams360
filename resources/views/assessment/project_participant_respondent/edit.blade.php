...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form
                action="/assessment/project-participant-respondents/{{ $projectParticipantRespondentAnswers[0]->project_participant_repondent_id }}"
                method="POST">
                @csrf
                @method('PUT')
                    ...
                    <tbody>
                        @for ($i = 0; $i < count($projectParticipantRespondentAnswers) - 1; $i += 2)
                            <tr>
                                ...
                                <td>
                                    <input type="number" class="form-control" min="1" max="10" maxlength="2"
                                        name="answers[]" value="{{ $projectParticipantRespondentAnswers[$i]->answer }}">
                                </td>
                                <td>
                                    <input type="number" class="form-control" min="1" max="10" maxlength="2"
                                        name="answers[]" value="{{ $projectParticipantRespondentAnswers[$i + 1]->answer }}">
                                </td>
                            </tr>
                        @endfor
                    </tbody>
