...
@section('content')
    @if (session('success-reset'))
        <div class="alert alert-success" style="margin-top: 1%">Akun {{ session('success-reset') }} berhasil ubah password
        </div>
    @endif
    <div class="card">
        <div class="card-body">
            <a href="/assessment/users/create" class="btn btn-info" style="margin-bottom: 1%">Tambah User</a>
    ...
    <div class="modal fade" id="ubah-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
        aria-hidden="true">
                ...
                <form action="" method="POST" id="myForm-ubah">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="alert alert-danger d-none" id="alert-name" style="margin-top: 1%">Harap isi field
                                nama</div>
                        </div>
                        <div class="form-group">
                            <label for="">Email</label>
                            <input type="text" name="email" class="form-control" id="email-modal">
                            <div class="alert alert-danger d-none" id="alert-email" style="margin-top: 1%">Harap isi field
                                email</div>
                        </div>
                        <div class="form-group">
                            <div class="form-group">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" id="btnUbah" class="btn btn-info">Ya</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="modal fade" id="ubah-pass-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <form id="myForm-pass" method="POST">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Tidak</button>
                        <button type="button" class="btn btn-info" id="btnReset">Ya</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="modal fade" id="hapus-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <form method="POST" id="myForm-hapus">
                    <div class="modal-body">
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="button" class="btn btn-secondary" data-dismiss="modal">Tidak</button>
                            <button type="button" id="btnHapus" class="btn btn-info">Ya</button>
                        </div>
                </form>
            </div>
        </div>
    ...
    <script type="text/javascript">
        $(function() {
            ...
            $('#pass-modal').on('keyup', function() {
                ...
                if (value_pass == value_conf && value_pass != "" && value_conf.length >= 8 && value_pass
                    .length >= 8) {
            ...
            $('#conf-modal').on('keyup', function() {
                ...
                $('#pass-modal').removeClass('is-invalid')
                $('#conf-modal').removeClass('is-invalid')
                if (value_conf == value_pass && value_conf != "" && value_conf.length >= 8 && value_pass
                    .length >= 8) {
                    $('#pass-modal').addClass('is-valid')
                    $('#conf-modal').addClass('is-valid')
                } else {
            ...
            $('#btnReset').on('click', function() {
                ...
                var value_pass = $('#pass-modal').val()
                var value_conf = $('#conf-modal').val()
                if (value_pass == value_conf && value_pass != "" && value_conf != "" && value_conf.length >=
                    8 && value_pass.length >= 8) {
                    $('#myForm-pass').attr('action', '/assessment/users/' + id)
                    $('#myForm-pass').submit()
                    ...
                    $('#empty').removeClass('d-none')
                } else if (value_pass != "" && value_conf != "" && value_conf.length >= 8 && value_pass
                    .length >= 8) {
                    $('#pass-modal').addClass('is-invalid')
                    $('#conf-modal').addClass('is-invalid')
                    $('#empty').addClass('d-none')
