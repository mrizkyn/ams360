                                    ...
                                    @if (count($participant) == 2)
                                        <tr>
                                            <td class="text-center"><input type="checkbox"
                                                    value="{{ $participant[0]->participant_id }}" name="participant_ids[]">
                                            </td>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#cetak').click(function() {
                $.post("{{ url('/data/dbassessment-participant-values') }}", $('#participant-form')
                        .serialize())
                    .done(function(results) {
                        ...
                        function competencyPopulationChart() {
                            var recommendationData = [
                                ['Kompetensi', 'Σ Asesi Naik', 'Σ Asesi Turun', 'Σ Asesi Tetap',
                                    'Σ Asesi Berubah'
                                ]
                            ];

                            results[3].forEach(function(competencyPopulation) {
                                recommendationData.push([competencyPopulation[0].competency,
                                    competencyPopulation[0].percent,
                                    competencyPopulation[1].percent,
                                    competencyPopulation[2].percent,
                                    competencyPopulation[3].percent
                                ]);
                            });
                        ...
                        function averageProgressChart() {
                            var averageProgressData = [
                                ['Type', 'Percent']
                            ];

                            results[5].forEach(function(averageProgress) {
                                averageProgressData.push([averageProgress.type, averageProgress
                                    .percent
                                ]);
                            });
                ...
                function getRecommendationData() {
                    $.post("{{ url('/data/dbassessment-recommendation-progress') }}", $('#participant-form')
                            .serialize())
                        .done(function(results) {
                            google.charts.load("45.2", {
                                packages: ['corechart']
                            ...
                            function recommendationChart() {
                                ...
                                var chart = new google.visualization.ColumnChart(document
                                    .getElementById("recommendationChartDiv"));
